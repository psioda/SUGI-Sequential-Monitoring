...
title;
ods html close;
ods pdf file="C:\Users\psioda\Documents\GitHub\SUGI-Session-4692\files\enthusiastic-prior.pdf" startpage=no;
...
proc iml;
	...
	do alpha = 1+delta to 10 by delta while(stop=0);
		beta = calc_beta(alpha,theta1);
		prob = cdf('beta',theta0,alpha,beta);
			if (beta>1) & 
